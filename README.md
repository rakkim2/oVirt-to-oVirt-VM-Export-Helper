...
이 문서는 oVirt VM to oVirt VM을 수행하는 `make_v2v_xml.sh`, `run_qemu_ova_pipeline.sh`, `import_v2v.sh` 기준 사용법입니다.  
...
## 0) 공통 설정 (import.conf + v2v.conf)

1. Export 계열 설정 파일: `import.conf`
   - 주 대상: `make_v2v_xml.sh`, `run_qemu_ova_pipeline.sh`, `convert_disks_from_xml.sh`, `make_ovirt_ova.sh`
   - 기본 XML 출력 경로: `/data/v2v/xml`

2. Import 계열 설정 파일: `v2v.conf`
   - 주 대상: `import_v2v.sh`
   - RHV/remote 연결 정보 관리

3. 기본적으로 스크립트는 자신의 기본 설정 파일을 자동 로드하며, 필요 시 아래처럼 교체할 수 있습니다.
   ```bash
   CONFIG_FILE=/path/to/file.conf bash script.sh ...
   ```
...
| Import 실행 (`import_v2v.sh`) | `bash import_v2v.sh --run-location=remote <VM_NAME>` | `CONFIG_FILE` (기본 `./v2v.conf`), `PRECHECK` (기본 `false`), `IMPORT_RUN_WITH_NOHUP` (기본 `true`) |
...
### `import_v2v.sh` (RHV import)
기본 실행(원격 타겟에서 import 실행):
```bash
CONFIG_FILE=/data/script/v2v.conf \
bash import_v2v.sh --run-location=remote ppcpap02
```

타겟 노드에서 직접 실행:
```bash
CONFIG_FILE=/data/script/v2v.conf \
bash import_v2v.sh --run-location=targetspm ppcpap02
```

체크 모드(실행 전 점검):
```bash
CONFIG_FILE=/data/script/v2v.conf \
bash import_v2v.sh --run-location=remote --check ppcpap02
```

| 옵션 | 기본값 | 설명 | 명령어 예시 |
|---|---|---|---|
| `--run-location` | 필수 | `remote` 또는 `targetspm` | `bash import_v2v.sh --run-location=remote ppcpap02` |
| `CONFIG_FILE` | `./v2v.conf` | import 설정 파일 경로 | `CONFIG_FILE=/data/script/v2v.conf bash import_v2v.sh --run-location=remote ppcpap02` |
| `PRECHECK` | `false` | import 전 precheck 실행 여부 | `PRECHECK=true bash import_v2v.sh --run-location=targetspm --check ppcpap02` |
| `IMPORT_RUN_WITH_NOHUP` | `true` | `run` 모드에서 백그라운드 실행 | `IMPORT_RUN_WITH_NOHUP=false bash import_v2v.sh --run-location=remote ppcpap02` |

`v2v.conf` 필수값(최소):
- `RHV_ENGINE_URL`
- `RHV_PASS_FILE`
- `REMOTE_TARGET_HOST` (remote 모드 사용 시)
...
### 4-4. import 실행 예시
```text
[2026-03-07 05:19:11] START import_v2v vm=ppcpap02 config=/data/script/v2v.conf
[2026-03-07 05:19:11] run location : remote
[2026-03-07 05:19:11] [3/3] run virt-v2v import
[2026-03-07 05:31:42] DONE import_v2v vm=ppcpap02
```
